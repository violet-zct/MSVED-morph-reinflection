...
class Layer(object):
    ...
    def get_output_mask(self, train=None):
        '''
        For some semi_models (such as RNNs) you want a way of being able to mark some output data-points as
...
class DropoutWord(MaskedLayer):
    """
        Hinton's dropout.
    """
    def __init__(self, rng=None, p=1., name=None):
        super(DropoutWord, self).__init__()
        self.p   = p
        self.rng = rng
    ...
    def __call__(self, X, train=True):
        if self.p > 0.:
            ...
            if train:
                mask = self.rng.binomial(mask_shape, p=retain_prob, dtype=theano.config.floatX)
                X *= mask[:, :, None]
        return X
