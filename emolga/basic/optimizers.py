...
class Optimizer(object):
    ...
    def get_gradients(self, loss, params):
        ...
        if isinstance(loss, list):
            grads = T.grad(loss[0], params, disconnected_inputs='ignore', consider_constant=loss[1:])  # gradient of loss
        else:
            grads = T.grad(loss, params, disconnected_inputs='ignore', return_disconnected='zero')
...
class Adam(Optimizer):  # new Adam is designed for our purpose.
    ...
    def get_updates(self, params, loss):
        ...
        for p, g in zip(params, grads):
            ...
            # g_noise = self.rng.normal(g.shape, 0, T.sqrt(0.005 * t ** (-0.55)), dtype=theano.config.floatX)
